...
jpegModelx::jpegModelx(BlockData& bd): MaxEmbeddedLevel(3),idx(-1),
    ...
    cpos(0), rs1(0), ssum(0), ssum1(0), ssum2(0), ssum3(0),cbuf2(0x20000),
    adv_pred(4),adv_pred1(3),adv_pred2(3),adv_pred3(3), run_pred(6),
    sumu(8), sumv(8), ls(10),lcp(7), zpos(64), blockW(10), blockN(10),
    SamplingFactors(4),dqt_state(-1),dqt_end(0),qnum(0),qnum16(0),qnum16b(0),pr0(0),
...
int jpegModelx::p(Mixers& m, int val1, int val2) {
    ...
    case 1: {
            int hc=1+(huffcode&1)*3;int p1=0;
            for (int i=0; i<N; ++i) {
                p1=p=hmap.p(i,hc,y); 
                const int n0=hmap.sn0(), n1=hmap.sn1();
                m1->add((p-2048)>>3); 
